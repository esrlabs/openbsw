...
inline block_pool::block_pool(
    uint8_t* const buffer, size_type const bufferSize, size_type const blockSize)
: _buffer(buffer)
...
inline bool block_pool::contains(uint8_t const* const object) const
{
    // suppress misra 5.0.15 next_construct: This pool class is intended to do low level memory
    // operations including pointer arithmetics
    return (object >= _buffer) && (object < (_buffer + _blockCount * _blockSize));
}

inline size_t block_pool::index_of(uint8_t const* const object) const
{
    // suppress misra 5.0.9 next_construct: All types are compatible
    return static_cast<size_t>(object - _buffer) / _blockSize;
}

inline uint8_t* block_pool::acquire()
{
...
inline void block_pool::release(uint8_t* const object)
{
...
inline void block_pool::clear()
{
    _firstFreeBlock            = nullptr;
    uint8_t* const bufferStart = _buffer;
    for (size_type idx = 0U; idx < _blockCount; ++idx)
    {
        // suppress misra 5.0.15 next_line: pointer arithmetics are required here
