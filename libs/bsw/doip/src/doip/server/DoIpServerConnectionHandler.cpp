...
bool DoIpServerConnectionHandler::resume(uint16_t const sourceAddress)
{
    if (_callback == nullptr)
    {
        Logger::info(
            DOIP,
            "DoIpServerConnectionHandler(0x%04x, 0x%d, %d)::resume(source: 0x%04x)",
            _sourceAddress,
            _state,
            _aliveCheckPending,
            sourceAddress);
...
void DoIpServerConnectionHandler::start(IDoIpServerConnectionHandlerCallback& callback)
{
    if (_callback == nullptr)
    {
        Logger::info(
            DOIP,
            "DoIpServerConnectionHandler(0x%04x, 0x%d, %d)::close()",
            _sourceAddress,
            _state,
            _aliveCheckPending);
        _callback = &callback;
...
void DoIpServerConnectionHandler::startAliveCheck()
{
    if (_state == State::ACTIVE)
    {
        Logger::info(
            DOIP,
            "DoIpServerConnectionHandler(0x%04x, 0x%d, %d)::startAliveCheck()",
            _sourceAddress,
            _state,
            _aliveCheckPending);
