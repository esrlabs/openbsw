...
FutureSupport::FutureSupport(ContextType const context) : 
_context(context),
_eventName("future")
{
    _eventName.append(std::to_string(context).c_str());
    tx_event_flags_create(
        &_eventObject, 
        const_cast<CHAR*>(_eventName.c_str())
    );
}
...
void FutureSupport::wait()
{
    while (true)
    {
        ...
        tx_event_flags_get(
            ...
            &events,
            TX_TIMER_TICKS_FROM_US(AsyncBinding::WAIT_EVENTS_US)
        );
