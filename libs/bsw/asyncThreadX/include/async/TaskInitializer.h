    ...
    TaskConfigType const& config)
{
    ...
    align(alignof(ULONG), bytes);
    StackSliceType const stackSlice = bytes.template reinterpret_as<ULONG>();
    estd_assert((stackSlice.size() * sizeof(ULONG)) >= sizeof(TaskInitializer));
