...
#define TX_TIMER_TICKS_FROM_US(us) (us / (1000000 / TX_TIMER_TICKS_PER_SECOND))
...
{
#if PRINT_DEBUG_LOG
    fprintf(stdout, "[%s] %u \t startTask()\n", getName(), getSystemTimeUs32Bit());
#endif
    ...
    RunnableType& runnable, TimeoutType& timeout, uint32_t const period, TimeUnitType const unit)
{
    if (!_timer.isActive(timeout))
    {
        ...
        timeout._context  = _context;
#if PRINT_DEBUG_LOG
        fprintf(stdout, "[%s] %u \t scheduleAtFixedRate()\n", getName(), getSystemTimeUs32Bit());
#endif
        if (_timer.setCyclic(timeout, period * static_cast<uint32_t>(unit), getSystemTimeUs32Bit()))
        {
            ...
            fprintf(
                stdout,
                "[%s]  %u \t scheduleAtFixedRate() -> setEvent()\n",
                getName(),
...
inline EventMaskType TaskContext<Binding>::waitEvents()
{
    ULONG events = 0;
#if PRINT_DEBUG_LOG
    fprintf(stdout, "[%s] %u \t waitEvents(): Entry\n", getName(), getSystemTimeUs32Bit());
#endif
    tx_event_flags_get(
        &_eventObject,
        WAIT_EVENT_MASK,
        ...
        TX_WAIT_FOREVER);
#if PRINT_DEBUG_LOG
    fprintf(stdout, "[%s] %u \t waitEvents(): Out\n", getName(), getSystemTimeUs32Bit());
#endif
...
void TaskContext<Binding>::setTimeout(uint32_t const timeInUs)
{
    ...
    fprintf(
        stdout,
        "[%s] %u \t setTimeout(): timeInUs %u \t tick: %u\n",
        getName(),
        getSystemTimeUs32Bit(),
        timeInUs,
        tick);
...
{
#if PRINT_DEBUG_LOG
    fprintf(stdout, "[%s] %u \t dispatch(): Entry\n", getName(), getSystemTimeUs32Bit());
#endif
...
#if PRINT_DEBUG_LOG
    fprintf(stdout, "[%s] %u \t dispatch(): Out\n", getName(), getSystemTimeUs32Bit());
#endif
}
...
{
#if PRINT_DEBUG_LOG
    static auto previous_time = getSystemTimeUs32Bit();
    auto const current_time   = getSystemTimeUs32Bit();
    auto const time_diff      = current_time - previous_time;
    previous_time             = current_time;
    fprintf(
        stdout,
        "[%s] %u \t handleTimerEventPolicy(): diff: %u\n",
        getName(),
        current_time,
        time_diff);
#endif
...
void TaskContext<Binding>::handleTimerFunction()
{
    ...
    fprintf(
        stdout,
        "[%s] %u \t handleTimerFunction(): diff: %u\n",
        _timerHandle->tx_timer_name,
        current_time,
        time_diff);
#endif
    _timerEventPolicy.setEvent();
