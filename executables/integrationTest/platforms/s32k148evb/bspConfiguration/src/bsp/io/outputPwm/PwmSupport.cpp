...
, test_pwm(ftm4, TEST_PWM_Configuration)
{}
...
void PwmSupport::init()
{
    ...
    (void)OutputPwm::setDynamicClient(OutputPwm::TEST_PWM, 2, this);
}
...
void PwmSupport::shutdown()
{
    ...
    (void)OutputPwm::clrDynamicClient(OutputPwm::TEST_PWM);
}
...
bsp::BspReturnCode PwmSupport::setDuty(uint16_t chan, uint16_t duty, bool /* immediateUpdate */)
{
    switch (chan)
    {
        ...
        case 2: test_pwm.setDuty(test_pwm.validDuty(duty)); break;
...
bsp::BspReturnCode PwmSupport::setPeriod(uint16_t chan, uint16_t period)
{
    switch (chan)
    {
        ...
        case 2:
        {
            (void)test_pwm.setlPeriod(period);
            break;
        }
