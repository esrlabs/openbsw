...
# Test: Checking positive response for did CF03
def test_WDBI_CF03(target_session, uds_transport):
    assert target_session.capserial().wait_for_boot_complete()

    uds_client = target_session.uds_client(uds_transport)
    did = 0xCF03
    val = "554AF2"
    didconfig = {0xCF03: ByteCodec()}
    req = uds.WriteDataByIdentifier().make_request(did, bytes.fromhex(val), didconfig)
    assert hexlify(uds_client.send_request(req).get_payload()[0:3]) == "6e cf 03"
...
def test_WDBI_F190(target_session, uds_transport):
    uds_client = target_session.uds_client(uds_transport)
    did = 0xF190
...
def test_ISO_INVALID_FORMAT(target_session, uds_transport):
    assert target_session.capserial().wait_for_boot_complete()
...
def test_ISO_REQUEST_OUT_OF_RANGE(target_session, uds_transport):
    assert target_session.capserial().wait_for_boot_complete()
